...
{
    public class AsuntosPenalesAdministradorUARepository : IAsuntosPenalesAdministradorUARepository
    {
        #region Variables
        ...
        #region Constructor
        public AsuntosPenalesAdministradorUARepository(ISqlTools database)
        {
        ...
        )
        {
            ...
            var response = await _database.ExecuteFunctionAsync(
                EnumFunctions.ASUNTOSPENALES_GET_HISTORICO_BY_FILTERS_ADMINUA,
                parameters!
            );
            if (response.ExisteError)
            {
        ...
        )
        {
            ...
            var response = await _database.ExecuteFunctionAsync(
                EnumFunctions.ASUNTOSPENALES_GET_HISTORICO_BY_FILTERS_ADMINUA_COUNT,
                parameters!
            );
            if (response.ExisteError)
            {
            ...
            foreach (DataRow item in response.Data.Tables[0].Rows)
            {
                resultList.Add(
                    new()
                    {
                        ...
                        oficio_solicitud = item.IsNull(4) ? null! : item.Field<string?>(4)!,
                        numero_expediente_cadido = item.IsNull(5) ? null! : item.Field<string?>(5)!,
                        id_unidad_realiza_solicitud = item.IsNull(6) ? null! : item.Field<int?>(6)!,
                        id_estado_tarea = item.IsNull(7) ? null : item.Field<int?>(7),
                        id_estado_procesal = item.IsNull(8) ? null : item.Field<int?>(8),
                        id_admin_controla = item.IsNull(9) ? null : item.Field<int?>(9),
                        id_administracion_central = item.IsNull(10) ? null : item.Field<int>(10),
                        fecha_turnado = item.IsNull(11) ? new() : item.Field<DateTime>(11),
                        activo = item.IsNull(12) ? false : item.Field<bool>(12)!,
                        numero_empleado = item.IsNull(13) ? null! : item.Field<string?>(13)!,
                        numero_empleado_adm = item.IsNull(14) ? null! : item.Field<string>(14),
                        fecha_asignacion = item.IsNull(15) ? new() : item.Field<DateTime>(15),
                        turnado = item.IsNull(16) ? false : item.Field<bool>(16),
                        usuario_modificacion = item.IsNull(17) ? null! : item.Field<string?>(17)!,
                        interno = item.IsNull(18) ? false : item.Field<bool>(18)!,
                        id_subadministracion = item.IsNull(19) ? null : item.Field<int>(19),
                        abogado = new()
                        {
        ...
        public async Task<ResponseAsuntosPenalesByIdAdminUA> GetAdministradorUAsuntoPenalById(
           int id_asuntopenal
       )
        {
            ParameterPGsql[] parameters =
            ...
            var response = await _database.ExecuteFunctionAsync(
                EnumFunctions.ASUNTOSPENALES_GET_ASUNTO_PENAL_ADMINUA_BY_ID,
                parameters
            );
            if (response.ExisteError)
            {
        ...
        public async Task<ResultTransaction> RemitirAsync(
